...
void run_tcp_server(int port) {
    ...
    std::cout << "Servidor TCP esperando conexiones en el puerto " << port << "..." << std::endl;
...
void run_udp_server(int port) {
    ...
    std::cout << "Servidor UDP esperando datagramas en el puerto " << port << "..." << std::endl;
...
void run_tcp_client(const char *server_ip, int port) {
    int sock = 0;
...
void run_udp_client(const char *server_ip, int port) {
    int sockfd;
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        std::cerr << "Uso: " << argv[0] << " <modo> <protocolo> <puerto> [<ip_servidor>]" << std::endl;
        std::cerr << "  modo: server | client" << std::endl;
        std::cerr << "  protocolo: tcp | udp" << std::endl;
        std::cerr << "  puerto: Puerto de escucha (ej: 8080)" << std::endl;
        std::cerr << "  ip_servidor: Obligatorio si modo=client (ej: 127.0.0.1)" << std::endl;
        return 1;
    }
    ...
    if (modo == "server") {
        ...
        if (protocolo == "tcp") {
            run_tcp_client(server_ip, port);
        } else if (protocolo == "udp") {
            run_udp_client(server_ip, port);
        } else {
            std::cerr << "Protocolo no válido. Use 'tcp' o 'udp'." << std::endl;
            return 1;
        }
